...
def send_email_change_confirmation_email(email: str, full_name: str, token: str) -> None:
    api_key = os.getenv("BREVO_API_KEY")
    email_from = os.getenv("EMAIL_FROM")
    frontend_url = os.getenv("FRONTEND_URL")
    ...
    verify_link = f"{frontend_url.rstrip('/')}/verify-email-change?token={quote(token)}"

    subject = "Confirm your new email for ClassmateAI"
    html_content = f"""
    <div style="font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; line-height: 1.5;">
      <p style="margin: 0 0 16px;">Hi {full_name or "there"},</p>
      <p style="margin: 0 0 16px;">
        We received a request to change the email on your ClassmateAI account.
        Please confirm this new email address to complete the update.
      </p>
      <p style="margin: 0 0 18px;">
        <a href="{verify_link}"
           style="display: inline-block; background: #00cc99; color: #ffffff; text-decoration: none; padding: 12px 18px; border-radius: 12px; font-weight: 600;">
